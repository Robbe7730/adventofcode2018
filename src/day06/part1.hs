...
main = do
    ...
    print $ score $ traceShowId $ toMap $ map readCoord $ lines input
...
toMap :: [Point2d] -> [[Maybe Point2d]]
toMap t = [[nearestNeighbor' t (Point2d (a, b)) | a <- [0 .. 500]] | b <- [0 .. 500]]

nearestNeighbor' :: [Point2d] -> Point2d -> Maybe Point2d
nearestNeighbor' t p
    | dist2 p (head sorted) == dist2 p (sorted !! 1) = Nothing
    | otherwise = Just $ head sorted
  where
    sorted = sortBy (comparePoints p) t
